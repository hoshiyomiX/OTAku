...
| File | Purpose |
|------|---------|
| `otaku-decomp` | **Bundled decompressor (T49)** — universal gzip/bzip2/xz/lz4/zstd + ZIP-entry extractor, first ZIP entry (stored, mode 755); verified by SHA-256 and self-tested before any partition is touched |
...
3. **Bundle integrity + decompressor wiring** — validates DDBU magic, version, compress_id, partition count; wires the already-verified bundled `otaku-decomp` (`$HELPER -a <alg>`) as the decompressor — no recovery-side decompressor binary is used or probed
...
8. **Flash each partition** — FIFO pipeline: `dd extract → otaku-decomp -a <alg> → dd write` (avoids tmpfs exhaustion for 4GB+ partitions)
...
- **No decompressor binaries needed** — since T49 the ZIP bundles its own universal decompressor (`otaku-decomp`, matched to the device ABI at build time: arm64-v8a / armeabi-v7a). gzip/bzip2/xz/lz4/zstd (and even `unzip`) on the recovery are never probed or required
