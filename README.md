...
| File | Purpose |
|------|---------|
| `otaku-decomp` | **Bundled decompressor (T49)** — universal gzip/bzip2/xz/lz4/zstd + ZIP-entry extractor, first ZIP entry (stored, mode 755); verified by SHA-256 and self-tested before any partition is touched. **T51**: also the parallel chunked-xz flasher (`--flash-chunked`) |
| `otaku.bin` | DDBU-format bundle: 4096-byte header + compressed partition data, 4096-aligned. **T51 (xz builds)**: DDBU **v2** — each partition is 64 MB independently-compressed chunks + a 24 B/entry chunk-table trailer (parallel encode in-app on up to 4 workers, parallel decode in recovery); non-xz builds stay byte-identical v1 |
...
1. **Open payload + bundled decompressor** — extracts `otaku-decomp` (entry #1) via local-header walk + bulk dd, verifies its SHA-256, runs its self-test; then walks entry #2 for direct ZIP read of `otaku.bin` (dd at a computed offset — ZIP-Stored, no extraction needed); falls back to the helper's own `--unzip-entry` (CRC-checked) on unusual ZIP layouts
...
3. **Bundle integrity + decompressor wiring** — validates DDBU magic, version (v2 chunked for xz, v1 otherwise — cross-checked against the script's baked expectation), compress_id, partition count; wires the already-verified bundled `otaku-decomp` (`$HELPER -a <alg>` for classic bundles, `$HELPER --flash-chunked` for chunked xz) — no recovery-side decompressor binary is used or probed
4. **Device compatibility** (optional) — compares `TARGET_DEVICE` against `getprop` + `build.prop` fallbacks; a detected mismatch aborts, an undetectable codename warns and proceeds (no interactive prompt — `choose`/`read` are unusable inside recovery's update-binary protocol pipe)
...
8. **Flash each partition** — classic bundles: FIFO pipeline `dd extract → otaku-decomp -a <alg> → dd write` (avoids tmpfs exhaustion for 4GB+ partitions); **chunked xz (T51)**: `otaku-decomp --flash-chunked` decodes 64 MB chunks on a parallel pool (compressed-hash verified inside the helper BEFORE any write) — ordered pipe through the same FIFO/dd machinery by default, or direct `pwrite` to the block device when `OTAKU_WRITE_MODE=pwrite` is set in the script
...
- **`busybox`** or **`toybox`** — provides `dd`, `awk`, `grep`, `mount`, `umount`, `blockdev`, `getprop`, `mkfifo`, `sha256sum`
- **No decompressor binaries needed** — since T49 the ZIP bundles its own universal decompressor (`otaku-decomp`, matched to the device ABI at build time: arm64-v8a / armeabi-v7a). gzip/bzip2/xz/lz4/zstd (and even `unzip`) on the recovery are never probed or required. Since T51, xz bundles even decompress **multi-threaded** (4-worker chunk pool) — no reliance on recovery-side `xz -T0` either
- **`/tmp` writable** — used for `otaku.bin` extraction and FIFO pipelines

> **Stock OEM recovery will NOT flash these ZIPs.** OTAku ZIPs carry no
> `META-INF/CERT.RSA`/`.SF`/`MANIFEST.MF` signature block, so a stock recovery
> rejects the package at its signature-verification gate before `update-binary`
> ever executes. Stock recovery ramdisks also lack the command set the script
> depends on (`od`, `awk`, `readlink`, `blockdev`, `mkfifo`, `sha256sum`, …) and
> never ship `lptools`. Flash via **TWRP**, **OrangeFox**, or **fastbootd**
