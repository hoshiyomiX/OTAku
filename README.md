...
1. **Open payload** — direct ZIP read (dd at a computed offset; `otaku.bin` is ZIP-Stored so no extraction is needed); falls back to extracting to `/tmp/` with ZIP-listing size verification on unusual ZIP layouts
